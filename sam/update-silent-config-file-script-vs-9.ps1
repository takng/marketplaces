...
$installer_name  = "SolarWinds-Orion-Install.exe"
...
if($xml.SilentConfig.Host.Info.Database)
{
    ...
    $dbnode.ServerName = $templateParameters['dbServerName']
...
if ($xml.SilentConfig.Host.Info.OrionLogConfiguration.StorageConfig -and (![string]::IsNullOrEmpty($olmproduct))) {
    ...
    $nodeStorageConfig.ServerName = $templateParameters['dbServerName']
...
if($xml.SilentConfig.InstallerConfiguration)
{
    ...
    $node.WebConsolePassword = $templateParameters['appUserPassword']
...
New-Item C:\cfn\artifacts\installer.ps1 -ItemType file
Add-Content 'C:\cfn\artifacts\installer.ps1' .\$installer_name" /s /ConfigFile=""$configfilePath"""

#Start installation
write-host ' starting installation solarwindinstaller....'; [datetime]::Now
Set-Location "C:\cfn\artifacts"
.\installer.ps1
...
while(1)
{
    $swInstallationStatus = Get-Process $process_name -ErrorAction SilentlyContinue
    write-host '  ============ Solarwinds installation status variable value ============= ' $swInstallationStatus; [datetime]::Now
    ...
    if ($swInstallationStatus) {
        ...
        write-host "Process completed in " $totalTime
        Remove-Variable swInstallationStatus
        break;
    }
}
...
$installer_file = "C:\cfn\artifacts\installer.ps1"
